            ...
            <div className="space-y-2">
               {history.length === 0 ? (
                 <div className="text-center p-8 border border-white/5 rounded-xl bg-white/[0.02]">
                    <p className="text-[10px] font-bold text-gray-600 uppercase tracking-widest leading-relaxed">No history found.<br/>Your briefs will appear here.</p>
                 </div>
               ) : (
                 history.map((item) => (
                   <div 
                     ...
                     className="group relative p-3 rounded-xl bg-white/5 border border-white/5 hover:border-[#BEFF00]/30 hover:bg-[#BEFF00]/5 transition-all cursor-pointer"
                   >
                     <div className="text-sm font-bold text-gray-200 group-hover:text-white truncate mb-1 pr-6">{item.name}</div>
                     <div className="text-[10px] text-gray-500 font-medium">{new Date(item.timestamp).toLocaleDateString()}</div>
                     <button 
                       onClick={(e) => deleteHistoryItem(e, item.id)}
                       className="absolute top-2 right-2 p-1 text-gray-600 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-all"
                     >
          ...
          <div className="flex items-center gap-4">
            ...
            {session ? (
              <div className="flex items-center gap-3">
                 ...
                 <button 
                   onClick={() => signOut()}
                   className="p-2 rounded-lg bg-white/5 hover:bg-red-500/10 text-gray-400 hover:text-red-400 border border-white/10 transition-all"
                   title="Sign Out"
                 >
              ...
              <button 
                onClick={() => signIn()}
                className="px-4 py-2 rounded-lg bg-[#BEFF00] text-black font-bold text-xs hover:bg-[#a5e000] transition-all shadow-lg shadow-[#BEFF00]/10"
              >
                Sign In
              </button>
            )}
        ...
        <div className="flex-1 flex flex-col lg:grid lg:grid-cols-10 overflow-hidden w-full max-w-[1600px] mx-auto p-4 lg:p-8 gap-6 lg:gap-8 relative min-h-[600px]">
          ...
          <div className="lg:col-span-4 h-full flex flex-col overflow-hidden bg-[#161616]/60 backdrop-blur-2xl border border-white/5 rounded-3xl shadow-2xl">
            ...
            <div className="flex-1 bg-white/95 text-gray-900 overflow-hidden flex flex-col relative">
               ...
               {formattedOutput && (
                  <div className="p-4 border-t border-white/10 bg-[#161616]/80 flex flex-col gap-3 justify-center backdrop-blur-md relative z-20 shrink-0">
                     <button 
                       onClick={() => {
                         handleCopy();
                     ...
                     <div className="flex gap-3 w-full">
                       <button 
                         ...
                         className="w-1/2 flex items-center justify-center gap-2 text-sm font-extrabold text-white bg-[#229ED9] hover:bg-[#1E8CC0] px-4 py-3.5 rounded-xl transition-all shadow-xl shadow-[#229ED9]/20 border border-transparent hover:border-white/20 active:scale-[0.98]"
                       >
                       ...
                       <button 
                         ...
                         className="w-1/2 flex items-center justify-center gap-2 text-sm font-extrabold text-white bg-[#25D366] hover:bg-[#20bd5a] px-4 py-3.5 rounded-xl transition-all shadow-xl shadow-[#25D366]/20 border border-transparent hover:border-white/20 active:scale-[0.98]"
                       >
